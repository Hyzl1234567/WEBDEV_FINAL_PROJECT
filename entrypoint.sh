...
echo "MESSENGER_TRANSPORT_DSN=sync://" >> /var/www/html/.env
echo "MAILER_DSN=${MAILER_DSN:-null://null}" >> /var/www/html/.env
echo "CORS_ALLOW_ORIGIN=^https?://.*$" >> /var/www/html/.env
echo "GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID:-placeholder}" >> /var/www/html/.env
echo "GOOGLE_CLIENT_SECRET=${GOOGLE_CLIENT_SECRET:-placeholder}" >> /var/www/html/.env
echo "PUSHER_APP_ID=${PUSHER_APP_ID:-placeholder}" >> /var/www/html/.env
echo "PUSHER_KEY=${PUSHER_KEY:-placeholder}" >> /var/www/html/.env
echo "PUSHER_SECRET=${PUSHER_SECRET:-placeholder}" >> /var/www/html/.env
echo "PUSHER_CLUSTER=${PUSHER_CLUSTER:-placeholder}" >> /var/www/html/.env
...
php bin/console doctrine:query:sql "CREATE TABLE IF NOT EXISTS sessions (sess_id VARCHAR(128) NOT NULL PRIMARY KEY, sess_data BLOB NOT NULL, sess_lifetime INTEGER UNSIGNED NOT NULL, sess_time INTEGER UNSIGNED NOT NULL) COLLATE utf8mb4_bin ENGINE = InnoDB;" --env=prod || true
